...
| Popup blocking | `<webview>`'s `new-window` event (fired for any `window.open()` call, including ad popunders) is not acted on - no new tab is opened for it - rather than the earlier behavior of auto-opening whatever the page requested | `src/renderer/renderer.js` |
