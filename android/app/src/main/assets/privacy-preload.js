...
/**
 * Injected into every page via `WebViewCompat.addDocumentStartJavaScript`
 * (see WebViewInjection.kt) - the androidx.webkit equivalent of Electron's
 * preload script guarantee: this runs before any of the page's own scripts,
 * in every frame, on every navigation.
 *
 * This is the same fingerprint-resistance logic as the desktop app's
 * src/preload/privacy-preload.js, adapted for WebView: the cosmetic/element-
 * hiding section at the end of that file (which used
 * `require('electron').ipcRenderer`) is handled natively instead here - see
 * CosmeticHiding.kt, which computes the selectors in Kotlin (where the URL
 * is already known) and pushes them in via `evaluateJavascript` from
 * `PrivacyWebViewClient.onPageFinished`, rather than exposing a
 * `addJavascriptInterface` bridge to every loaded page just to ask it.
 *
 * This is best-effort, JS-level fingerprint resistance. It raises the bar
 * for casual/commercial trackers but is not equivalent to Tor Browser's
 * years of hardening at the engine level - see docs/THREAT_MODEL.md and
 * android/README.md.
 */
(function privacyHardening() {
  const safeDefine = (obj, prop, getter) => {
    try {
      ...
      // Some properties may already be non-configurable in this WebView
  ...
  // video/camera constraints are refused. This mirrors
  // PrivacyWebChromeClient.onPermissionRequest's native-level denial, so the
  // policy holds even if a page tries to call the API before a permission
  // prompt would fire.
  ...
  // like Tor Browser's window-size quantization). ---
  ...
  try {
    ...
    // of the window instance in this engine (not on Window.prototype), so
    // the override has to target `window` directly.
